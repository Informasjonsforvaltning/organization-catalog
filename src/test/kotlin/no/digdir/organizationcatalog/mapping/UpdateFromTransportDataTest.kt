...
class UpdateFromTransportDataTest {
    ...
    val transportOrganization: TransportOrganization = TransportOrganization(
        id = "transp-123",
    ...
    fun `Should create if there is no transport data in DB`() {
        ...
        assertEquals(updatedTransportDataDB.prefLabel?.nb,
    ...
    fun `Should not update if the new downloaded data does not have a company name`() {
        ...
        var updatedTransportDataDB = transportOrganization
        ...
        updatedTransportDataDB = transportOrganization
            .copy(tradingName = " ")
            .updateOrCreateTransportData(initialTransportDataDB)
